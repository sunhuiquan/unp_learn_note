...
会造成永久阻塞，32位客户机发送2两个32位值到缓冲区，64位服务器等待两个64位值，而缓冲区的两个32位值被接受到一个64位值取走，剩下一个64位值的读取会永远阻塞。

### 5.11

IP路由功能，根据目的IP地址，查路由表确定外出接口和下一跳，这个接口的主IP地址用做源IP地址，前提是未绑定bind到一个地址，bind后则内核不再分配。
