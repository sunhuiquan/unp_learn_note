...
[代码实现](./exercise/ch7/1/demo.c) 注意下这个是默认值，建立连接后如果不是MSS的倍数可能会被内核自动向上舍入到MSS的倍数。

### 7.2

<!-- to do -->
MSS建立连接前是默认值(一般是536)，建立连接后,(准确来说是第一次接受到对端的SYN报文后),得到真实的MSS值并设置。  
而接收缓冲区建立连接前是默认值，建立连接后获得了MSS值，而接收缓冲区大小应该是MSS的倍数(一些优化问题的结果)，如果之前的接受缓冲区大小默认值不符合MSS倍数就可能被(大部分实现)内核自动向上舍入成MSS倍数。
